...
if(SECURE){
	wss = new WebSocketServer({ server: createServer({cert: await fs.readFile('/etc/letsencrypt/live/server.placetest.tk/fullchain.pem'),
	key: await fs.readFile('/etc/letsencrypt/live/server.placetest.tk/privkey.pem') }).listen(1291) })
...
async function pushImage(){
	...
	await new Promise((r, t) => exec("git add *;git commit -m 'Hourly backup';git push "+ORIGIN+"/pedroporo/placetest", e => e ? t(e) : r()))
